...
//Animates the scene
std::vector<View> Scene::Animate(float timeFrame, int fps)
{
	int frameCount = (int)floor(timeFrame * fps);
	float deltaScale = 1.0 / (frameCount-1);
...
Point Scene::RayTrace(Ray ray, float refractionIndex, int level)
{
	...
	{   
		...
		{			
			...
			if (!shadowCollision.IsCollision || shadowCollision.Distance > lightDistance)
			{
...
Collision Scene::Trace(Ray ray)	
{
	...
	{			
		Primitive* obj = (*index);    
		//Create new ray and reverse transform it
		Ray intersectRay = Ray(ray.Origin, ray.Direction);

		std::vector<TransformationMatrix*>::iterator tIndex = obj->Transformations.begin();
		std::vector<TransformationMatrix*>::iterator tEnd = obj->Transformations.end();
		for(; tIndex != tEnd; tIndex++)
		{
			TransformationMatrix* t = (*tIndex);

			TransformationMatrix transform = t->ScaleMatrix(TransformationScalingFactor);

			TransformationMatrix invTransform = transform.GetInverse();
			Vector3DMatrix dirMatrix;
			Vector3DMatrix originMatrix;

			dirMatrix.FromVector(intersectRay.Direction);
			originMatrix.FromPoint(intersectRay.Origin);

			Vector3DMatrix invDir = dirMatrix.Transform(invTransform);
			Vector3DMatrix invOrigin = originMatrix.Transform(invTransform);
		...
		Collision c = obj->Intersection(intersectRay);
		...
		if (c.IsCollision && c.Distance < minT)
		{
			...
			std::vector<TransformationMatrix*>::iterator tIndex2 = obj->Transformations.begin();
			std::vector<TransformationMatrix*>::iterator tEnd2 = obj->Transformations.end();
			for(; tIndex2 != tEnd2; tIndex2++)
			{
				TransformationMatrix* t2 = (*tIndex2);

				TransformationMatrix transform = t2->ScaleMatrix(TransformationScalingFactor);

				Vector3DMatrix normalMatrix;
				Vector3DMatrix hitpointMatrix;

				normalMatrix.FromVector(hit.Normal);
				hitpointMatrix.FromPoint(hit.HitPoint);

				Vector3DMatrix invNormal = normalMatrix.TransformNormal(transform);
				Vector3DMatrix invHitpoint = hitpointMatrix.Transform(transform);
